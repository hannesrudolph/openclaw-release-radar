...
`npm run doctor` is a read-only SQLite health report. It prints DB path/size, table counts, latest scored stable, recommendation count, score-write provenance, classification coverage, source freshness, closure-proof coverage, PR reachability counts, comparison snapshot state, warnings, and failures. It also checks every scored stable release row against its matching `release_score_audits` row, including missing audits, audit rows without scored release rows, release/audit drift in score, score timestamp, status, or recommended flag, and exact score-source identity drift. It checks the latest recorded issue crawl metadata, including missing crawl metadata on scored DBs, page-cap stops, truncated comment scans, issue classification failures, release-metadata/artifact/release-check/advisory/monitored-release evidence refresh failures, durable `ingestion_evidence_failures` rows recorded after the latest score, malformed freshness timestamps, issue comment snapshot freshness, and source rows that changed after the latest score. Refresh treats truncated comment scans as score-blocking incomplete evidence before score persistence, because commenter breadth and comment-derived proof must be complete enough to audit. Use `--api-base http://127.0.0.1:8787` to also check the running local server. Add `--fail-on-warnings` when stale evidence warnings should fail the command.
...
Score writers also enforce complete classification coverage, unambiguous stable release windows, unambiguous issue open intervals, and a stable source snapshot before updating release rows or score audits. Score construction runs inside one SQLite read transaction, hashes explicit ordered columns from the 15 score-input tables before and after analysis, then verifies the same identity again inside the score-write transaction. If any release has `classifiedIssueCount !== rawIssueCount`, a stable release has a missing/duplicate `published_at`, a fetched reopen event has no preceding close event for that issue, or any source row changes during analysis or before persistence, `persistReleaseScoreRun` refuses the whole run. Every successful audit stores `source_identity_json`, containing SHA-256 per-source digests and a combined database identity. `score_persistence_last_run` schema v2 records that digest plus writer source, scope, release tags, score timestamps, recommended tag, model/prompt versions, and issue-crawl coordinates. Doctor fails if the current scored stable rows or audited stable rows do not match that recorded release tag set, model version, prompt version, or source identity, and it checks every scored release/audit pair for scalar drift.
...
| `/api/releases/:tag/review` | Local score audit for one release; no upstream comparison fields; includes `local.sourceProvenance` with the persisted score-source identity, score/audit timestamp alignment, freshness sources, and raw-row links |
...
npm run backfill:issue-comment-snapshots -- --all-scored
...
`npm run backfill:issue-comment-snapshots -- --all-scored` fetches comments for the current scored release issue universe and writes `issue_comment_snapshots` rows with total/fetched comment counts, latest comment update time, and a digest. These snapshots give commenter breadth, classification context, and closure-comment proof their own freshness source. Run `npm run verify:local` afterward; if snapshots are newer than the current score, rerun scoring.
...
Refresh computes closure proof automatically. During refresh and closed-window backfill, closure proof updates side-table proof rows but leaves `release_score_audits` unchanged until the final score write succeeds, so a failed evidence pass cannot attach fresh proof payloads to stale scores. `npm run analyze:closure-proofs -- <tag>` reruns closure evidence, PR reachability, and proof for that tag, then rebuilds all monitored scores/audits in one final score transaction so the score-source identity remains valid. It requires clean ingestion metadata before writing, requires the release to exist locally, and records score-blocking `ingestion_evidence_failures` if the proof or score pipeline aborts.
