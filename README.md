...
| `/api/releases/:tag/review/issues` | Paginated issue-evidence rows for one release; supports comma-separated `tier`, `impact`, `state`, plus `limit` and `cursor`; includes `tierInfo`, `summaryByTier`, and `filteredSummary` |
