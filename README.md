...
The `/api/releases/:tag/review` `local.issueEvidence` object also exposes `schemaVersion`. Current value: `3`.
The `/api/releases/:tag/review` `local.gateEvidence` object also exposes `schemaVersion`. Current value: `1`.
The embedded `local.gateEvidence.fixProvenance.closureProof` object exposes `schemaVersion`. Current value: `2`; the sibling `releaseFixCredit` payload remains at schema version `1`.
