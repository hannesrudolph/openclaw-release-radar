...
| `/api/releases/:tag/review/issues` | Paginated issue-evidence rows for one release; supports comma-separated `tier`, `impact`, `state`, `sentiment`, `severity`, `functionality`, `scope`, `affectedUsers`, `fieldConfirmed`, `minWeight`, `maxWeight`, plus `limit` and `cursor`; includes `tierInfo`, `summaryByTier`, and `filteredSummary` |
