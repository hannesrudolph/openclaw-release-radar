...
SELECT * FROM releases ORDER BY published_at DESC NULLS LAST LIMIT ?
`);

export function listReleasesDb(limit = 20): ReleaseRow[] {
  return listReleasesStmt.all(limit) as ReleaseRow[];
}
...
export function issuesForVersion(tag: string): JoinedIssue[] {
  return issuesForVersionStmt.all(tag) as JoinedIssue[];
}
...
export function issuesWithoutVersion(): JoinedIssue[] {
  return issuesWithoutVersionStmt.all() as JoinedIssue[];
}
