...
Every score write also records `meta.score_persistence_last_run` in the same transaction as release rows and `release_score_audits`. The record captures the writer source, scope, release tags, recommended tag, model/prompt versions, score timestamps, and issue-crawl coordinates used by that writer. Doctor fails if current score rows drift from this provenance, including the exact release-tag set, model version, and prompt version, and it compares every scored stable release row with its audit row for final score, score timestamp, status, and recommended flag. Missing audit rows, audit rows without scored stable release rows, and per-release scalar drift all fail the health report so manual score writes such as `populate-db` or `backfill:closed-windows` are auditable instead of silently replacing refresh-produced scores.
