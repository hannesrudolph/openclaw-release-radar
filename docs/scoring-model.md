...
Current model: `evidence-v29-authority-reference-manifest`
...
Normal incremental runs remain `UPDATED_AT DESC` and are anchored to the stored baseline, so later-created and newly changed issues are picked up without entering the historical first-N baseline. Early-stop requires every remote metadata field on the page to match storage; metadata-only title, body, state, label, reaction, comment-count, or timestamp changes prevent the stop. A naturally exhausted incremental run is labeled `incremental_exhaustive`, remains unstabilized, and retains the stored as-of boundary. Refresh validates repository identity, baseline binding, count/growth arithmetic, cursors, mode, and digest semantics immediately before score persistence; doctor and release-audit verification enforce the same contract.
...
- `affectedUsers` is derived only from explicit population or configuration scope in issue/comment text. Prompt version 9 requires exact `{source_id, excerpt}` citations in `evidence.affected_users` for `many`, `some`, or `few`; `unknown` requires an empty citation array. Reporter identities, duplicate reports, comment volume, commenter counts, reactions, "+1", "me too", and reproduction volume are excluded from the classifier prompt as affected-user evidence.
...
Issue-evidence schema v2 also stores uncapped `evidenceCounts` beside the capped example arrays. Validation and audit tooling use those totals to detect truncated field/debt/fix evidence rather than comparing one capped list with another.
...
The first evidence build must run with `FULL_ISSUE_BACKFILL=true`. Normal refresh may stop after an unchanged page or after crossing the monitored cutoff only after an exhaustive, stabilized issue catalog has established both the issue crawl baseline and the verified state-event evidence required by later incremental refreshes.
...
Resume validates the complete snapshot chain, current row schema, canonical payloads, row count, row and aggregate hashes, membership/content digests, repository, and age. Required score-overlap rows still fetch current full issue metadata plus comments, labels, and state evidence; drift is reconciled before persistence. Rows created after the frozen boundary remain outside the staged catalog and are intentionally collected by the subsequent incremental crawl. Catalog staging alone never establishes `issue_crawl_exhaustive_baseline` or `backfill_completed_at`; every staged page must complete evidence and classification first.
...
`/api/releases/:tag/review` also exposes `local.issueEvidence.schemaVersion`. Current value: `2`.
