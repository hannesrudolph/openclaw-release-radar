...
Prompt version 9 requires every classification enum, rationale, and the exact `evidence` object keyed by all score-affecting fields. Known affected-user scope requires at least one exact `{source_id, excerpt}` citation from the included issue title, body, or comments; `unknown` requires an empty citation array. The model does not output `confidence` or `hasWorkaround`: the caller derives confidence deterministically from verified citation quality and derives `hasWorkaround` from `workaroundStatus`. Missing, extra, invalid, inconsistent, ungrounded, or non-JSON model output is not defaulted into a scoreable row. Model-correctable grounding failures receive bounded value-specific feedback using only the latest rejected output; repeated unsupported values must change their evidence or value, and scope retries receive exact included-source examples for every mechanically supported rung. A narrowly worded feature is not `niche` unless the cited text limits impact to a specialized or non-default population or setup. Every raw attempt remains in the classifier ledger. The response model and service tier must exactly match the request.
