...
Prompt version 10 requires every classification enum, rationale, and the exact `evidence` object keyed by all score-affecting fields. Known affected-user scope requires at least one exact `{source_id, excerpt}` citation from the included issue title, body, or comments; `unknown` requires an empty citation array. The model does not output `confidence` or `hasWorkaround`: the caller derives confidence deterministically from verified citation quality and derives `hasWorkaround` from `workaroundStatus`. Eligible structural citation defects may be normalized only by filtering or selecting exact field-relevant citations that the model supplied in that same evidence field. Normalization never synthesizes an excerpt, substitutes a source, or moves a citation between fields; cross-field citation reuse requires retry or rejection. Provenance records the original and effective evidence plus normalization diagnostics. Missing, extra, invalid, inconsistent, non-JSON, or non-normalizable grounding output remains unscoreable and follows the bounded retry/rejection path. For every rejected mandatory field, the retry payload enumerates exact contextual source excerpts accepted by the deterministic validator. When a diagnostic identifies a source, retry searches that source first; if it yields no supported value, retry scans every other already-included source in canonical order. When an under-ranked severity cites text that directly supports a different severity, retry binds that exact citation to the highest supported severity. The retry response schema narrows that field to the enumerated values when the list is nonempty, and every prior field restriction remains active on later attempts. Retry feedback also retains one bounded active correction requirement per previously diagnosed field; fresh diagnostics replace the requirement for that field, while unrelated prior requirements remain marked `carried` until the complete classification is accepted. Requirements are canonically ordered, filtered only after enum constraints merge, and fail closed on conflicts or size limits. Only the latest rejected output and latest diagnostics are serialized. An empty support list leaves the original enum intact so the validator still fails closed rather than inventing a replacement. The replacement must choose from the listed values, copy exact included-source evidence, and keep mandatory citation identities distinct. Repeated unsupported values must change their evidence or value. Explicit problem, leak, frustration, dropped/ignored values, and timeout language can support negative sentiment, while negated forms such as "no leakage" or "not a problem" cannot. Exact Chinese inability, non-working, incomplete-result, and returned-only-result phrases are also accepted without translation. Explicit compromise language supports critical security severity. Explicit proposal phrasing includes contracted requests such as "I'd like to discuss"; generic discussion wording alone is not enough. Explicit deterministic-package requests phrased as making package or file order deterministic are also neutral. Explicit suggestion wording such as "would be great to expose" can support low severity. Deterministic or reproducible package/archive-order cleanup, exact cleaner-diff benefits for packaged artifacts, and explicit edge cases support low severity. Bare template text such as `Agent request:`, `Example:`, or `Proposed solution` does not establish severity or documentation functionality. A `Severity: critical|high|medium|low` declaration controls only the citation that contains it; a declaration elsewhere in the issue does not override a different cited excerpt. A narrowly worded feature is not `niche` unless the cited text limits impact to a specialized or non-default population or setup. Exact `skill-creator` or `package_skill.py` evidence identifies a niche developer-packaging surface; numeric-string/non-finite handling and LiteLLM-to-Anthropic proxy paths are also specialized evidence. Explicit multi-instance or high-load production deployments are specialized setups and may support `niche`; this does not make generic mentions of multiple sessions or options niche. Unpredictable performance under load, repeated timeout evidence, garbled or malformed binary output, large-scale token waste, significant reasoning degradation, exact Chinese incomplete-fetch/returned-record evidence, or explicit operational risk/complexity can support medium severity. Modifiers such as `user-specific` or `Anthropic-specific` do not independently prove medium severity or a few-user population. Explicit alternatives described as working but heavyweight, cumbersome, impractical, fragile, manual, monkey-patched, technically working before a stated limitation, or requiring several installs/apps/steps support a partial workaround. Additional or multiple configuration/state layers inside one product domain can support moderate scope. One explicitly named messaging integration such as Telegram, Slack, Discord, or Synology Chat supports moderate scope. Singular or plural `messaging channel` evidence supports integration functionality. Audio-file, read-tool, WebUI, and Control UI evidence also identifies a moderate product surface; WebUI and Control UI are integration functionality. Exact Feishu plugin, `feishu_wiki`, and `@openclaw/feishu` identifiers support both moderate scope and integration functionality. A `Scope: ... only` declaration independently supports `niche` only when the same citation explicitly identifies i18n, l10n, internationalization/internationalisation, localization/localisation, locale, or language scope; generic subsystem or component wording is insufficient and does not prove that few users are affected. Doctor, upgrade, read-tool, `sessions_spawn`, and runtime workspace document write/commit behavior remains `core` functionality even when the feature is described as linting; `tooling` requires explicit test, CI, build, lint, formatting, fixture, harness, developer-infrastructure, `CI and release workflows`, or the exact model-usage helper path without a runtime target. A bare `model` token in a tooling path is not provider evidence. One named tool-call, `exec`, subagent, `sessions_spawn`, doctor, upgrade-scanner, or pre/post-update surface is `moderate`, not `broad`; broad scope also accepts explicit every-instance or any-browser/any-device reach and otherwise still requires explicit broad or broader multi-surface, multi-provider, or multi-platform evidence, including an explicit list of at least three named runtime surfaces such as exec, filesystem, session, CLI, slash, cron, Gateway, node, or startup-readiness. Conditional populations such as "anyone updating" support `affected_users=some` without being promoted to all-user impact. Every raw attempt remains in the classifier ledger. The response model and service tier must exactly match the request.

An exhaustive refresh accumulates deterministic, model-correctable grounding failures across the remaining issue crawl while durably persisting successful sibling classifications. Any accumulated failure still blocks score publication. Cancellation, recorder failures, transport failures, malformed response envelopes, and other non-grounding failures remain fail-fast so a systemic outage cannot trigger an unbounded classifier sweep.
...
Canonical closure resolution is transitive. Discovery follows trusted canonical edges at every hop, fetches terminal canonical issues that sit outside the source release window when required, and includes source issues, every reachable canonical issue, and terminal evidence-only issues in the analysis set. Cross-release fallback proof is admitted only from an active stable GitHub release. After proof construction, `release_closure_dependency_snapshots` stores that exact sorted issue set plus an analyzer version, row count, and digest over the release/commit, issue, classification, comment, state, label, closure/reopen, PR/commit link, PR metadata, reachability, and active-stable cross-release proof and release-identity rows. Any dependency-set, release eligibility, release identity, or digest change makes the proof stale and blocks score persistence until it is rebuilt.
