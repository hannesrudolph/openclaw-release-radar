...
Each release review also exposes `dataFreshness`. `scoredAt` is when the score/audit payload was computed. `issueUpdatedAtMax` is the newest GitHub issue `updated_at` value included in the release issue universe. `issueUpdatedAgeHoursAtScore` makes that semantic GitHub gap explicit, while `issue_fetches` records when those issue rows were last fetched/written locally. `issue_comment_snapshots` records fetched comment counts, latest comment update time, a comment digest, and fetch time for each fetched issue so commenter breadth, classification context, and closure-comment proof have durable freshness provenance without storing another full comment corpus. Each source exposes `count`, `nullCount`, `maxAt`, and age-at-score data; populated freshness sources must not have null or malformed timestamps, otherwise doctor treats the score as not fully auditable. A freshly computed score can still be based on issue rows, labels, comments, or source metadata fetched earlier. The review UI shows this as `Source freshness`, and the API includes per-source timestamps for release-row metadata/artifact checks, issue rows, local issue fetches, issue comment snapshots, classifications, label events/snapshots, closure proof, closure events, PR links, PR metadata, and release reachability.

Every score run also computes a schema-versioned `source_identity_json` manifest over explicit ordered columns from `releases`, `release_commits`, `advisories`, `issues`, `classifications`, issue comment/label/closure/reopen evidence, PR links, commit references, PR metadata, release reachability, and closure proof rows. Comparison tables and score-output columns are excluded. The manifest stores a SHA-256 digest for each source plus a combined digest. Score construction runs inside one SQLite read transaction, computes the identity before and after analysis, and refuses the run if they differ; persistence recomputes it inside the write transaction and refuses to write if source rows changed after analysis. `doctor`, `verify:score`, and `verify:release-audit` recompute the same manifest and fail on missing, malformed, or mismatched identities, including in-place row edits that preserve row counts and timestamps.
...
Every score write also records `meta.score_persistence_last_run` schema v2 in the same transaction as release rows and `release_score_audits`. The record captures the writer source, scope, release tags, recommended tag, model/prompt versions, score timestamps, issue-crawl coordinates, and score-source identity schema/digest/counts used by that writer. Doctor fails if current score rows drift from this provenance, including the exact release-tag set, model version, prompt version, or source identity, and it compares every scored stable release row with its audit row for final score, score timestamp, status, and recommended flag. Missing audit rows, audit rows without scored stable release rows, and per-release scalar drift all fail the health report so manual score writes such as `populate-db` or `backfill:closed-windows` are auditable instead of silently replacing refresh-produced scores.
...
Refresh recomputes closure proof automatically for monitored releases. The refresh path writes proof rows with score-audit payload persistence disabled and relies on the subsequent score transaction to attach the matching proof payload. The manual full-provenance commands rerun the same proof pass for a specific tag, keep proof writes side-table-only during analysis, then rebuild all monitored scores/audits in one final score transaction so the persisted source identity matches the refreshed evidence. They require the release to exist locally, require clean ingestion metadata before writing, and record score-blocking `ingestion_evidence_failures` rows when the proof, reachability, or score pipeline aborts.
